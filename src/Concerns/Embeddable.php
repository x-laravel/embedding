...
trait Embeddable
{
     ...
     * (via MissingSlotResolver), so the count never drifts from reality.
     */
    public static function missingEmbeddingCount(?string $slot = null): int
    {
        ...
        foreach ($slots as $slotName) {
            $missing += MissingSlotResolver::for(static::class, $slotName)->count();
        }
    ...
    /**
     * Constrain a model query the way a payload filter constrains a similarity
     * search: scalars compare as equality, arrays as OR, null matches only a
     * null column, and an empty array matches nothing. One filter definition
     * can therefore drive both similarTo(filter: ...) and a plain query — the
     * candidate pool a search sees and the pool a listing shows stay in sync.
     *
     * Keys must be #[EmbedPayload] columns; a value computed inside
     * toEmbeddingPayload() has no column to compare against. Comparison here
     * is the database's, not the payload store's: a column and its payload
     * copy can differ in type strictness.
     *
     * @param  Builder<static>  $query
     * @param  array<string, mixed>  $filter
     * @return Builder<static>
     */
    public function scopeWherePayloadFilter(Builder $query, array $filter): Builder
    {
        $wildcard = static::embeddingPayloadAttribute()?->isWildcard() ?? false;
        $fields = $this->embeddingPayloadFields();

        foreach ($filter as $column => $value) {
            if (! $wildcard && ! in_array($column, $fields, true)) {
                throw new InvalidArgumentException(sprintf(
                    'Payload filter key [%s] is not an #[EmbedPayload] column on [%s].',
                    $column,
                    static::class,
                ));
            }
            ...
            if (is_array($value)) {
                ...
                $query->where(function (Builder $group) use ($column, $value) {
                    foreach ($value as $item) {
                        $item === null ? $group->orWhereNull($column) : $group->orWhere($column, $item);
                    }
                });
            } elseif ($value === null) {
