...
interface PayloadStoreMetrics
{
    /**
     * Return a driver-specific snapshot of the payload (embeddables) storage.
     *
     ...
     * privileges) must return `null` and let `embedding:payload:status`
     * render "n/a". Implementations must not throw to signal "not
     * supported"; reserve exceptions for genuine I/O failures.
     *
     ...
     * exact count, callers can always fall back to `Embeddable::count()`.
     *
     * @return array{
     *     rows: int,
     *     bytes: int|null,
