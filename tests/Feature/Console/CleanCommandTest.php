...
class CleanCommandTest extends TestCase
{
    ...
    public function test_cleans_vector_and_payload_tables_in_one_pass(): void
    {
        Embedding::create([
            'embeddable_type' => 'App\\Models\\Ghost',
            'embeddable_id' => 1,
        ...
        $this->artisan('embedding:clean', ['--force' => true])
            ...
            ->expectsOutput('Deleted 2 record(s).')
            ->assertSuccessful();
    ...
    public function test_does_not_accept_filter_options(): void
    {
        // Single-table filters live in embedding:vector:clean /
        // embedding:payload:clean — the umbrella command must reject them.
        $this->expectException(\Symfony\Component\Console\Exception\InvalidOptionException::class);

        $this->artisan('embedding:clean', ['--orphans-only' => true]);
    }
