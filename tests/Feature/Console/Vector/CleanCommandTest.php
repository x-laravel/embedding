...
class CleanCommandTest extends TestCase
{
    ...
    {
        $this->artisan('embedding:vector:clean', [
            '--orphans-only' => true,
    ...
    {
        $this->artisan('embedding:vector:clean', ['--force' => true])
    ...
    public function test_orphans_only_skips_invalid_slot_records(): void
    {
        ...
        $this->artisan('embedding:vector:clean', ['--orphans-only' => true, '--force' => true])
            ->expectsOutput('Deleted 1 embedding(s).')
            ->assertSuccessful();
    ...
    public function test_invalid_slots_only_skips_orphans(): void
    {
        ...
        $this->artisan('embedding:vector:clean', ['--invalid-slots-only' => true, '--force' => true])
            ->expectsOutput('Deleted 1 embedding(s).')
            ->assertSuccessful();
    ...
    public function test_never_touches_stale_payload_rows(): void
    {
        Embedding::create([
            'embeddable_type' => 'App\\Models\\Ghost',
            'embeddable_id' => 1,
        ...
        // Stale payload rows belong to embedding:payload:clean.
