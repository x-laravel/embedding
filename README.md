...
The package's major version follows `laravel/ai`: `x-laravel/embedding` 1.x targets `laravel/ai` 1.x.
...
Idempotent — records that already have a payload row are skipped unless `--force` is given. Use it as the backfill step after adding `#[EmbedPayload]` to a model with existing records. Payload-less models warn and count 0.
